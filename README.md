...
The fixed Alpha commands are documented in the [Codex connection guide](docs/CODEX_CONNECT.md). Registry commands become usable only after the packages and `v0.1.0-alpha.6` ref are published. Before that, collaborators with private repository access use the source-checkout path in the same guide.
...
### Unreleased source preview: project code sync and native context management

The current source adds opt-in Git-backed project code checkpoints: a separate branch per member, manual/idle automatic upload, clean download, new-directory recovery, change review and owner-only merge. Codex requires explicit `--code-sync` authorization; DSH exposes project code consent and controls in its plugin settings. Existing conversation uploads and context injection remain independent. This does not alter your original Git branch/index or provide a public `git push` endpoint. All project readers can read code branches, including branches associated with Solo work. See the bilingual [setup, limitations and acceptance checklist](docs/CODE_SYNC.md). **These changes are not yet in the published Alpha 6 npm packages.**

The source also fixes native context accounting and removes application-side clipping of accepted incoming history. Codex prioritizes its reported model capacity; DSH keeps native model/compaction settings. Native summaries are not lossless or unlimited, and exceptional recovery limits still apply. These audit/context fixes are also unreleased; see [Codex context management](docs/CODEX_CONNECT.md#native-context-management-unreleased-source-fix) and [DSH guidance](docs/DSH_CONNECT.md).

The source preview additionally lets a session **writer** select completed public messages and ask their **own connected local Agent** for a shared manual summary. The new version is attributed and visible to project members; no original event is deleted. Summaries can be selected again as source text for a later summary, and the Web UI can switch between the compact view, original messages, and earlier versions. A per-user project setting chooses summarized context by default or original text for future Web-triggered Agent requests and explicit derived-context reads; it does not retroactively rewrite a native Codex/DSH conversation. Writers can customize/reset the summary instructions. Viewers can read and switch views but cannot generate summaries. This is separate from each harness's native compaction and from local-turn upload consent. See [ADR-0027](docs/adr/0027-shared-manual-history-summaries.md). **These changes are not in the published Alpha 6 packages.**
