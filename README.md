...
**[Open an Alpha access Issue](https://github.com/TH060419/gatherthread/issues/new?template=test-access.yml)** to request a test qualification. This Issue is the only application channel; no email form or application profile is required. While the repository is private, the Issue link is accessible only to people with repository access; public requests become possible after the owner makes the repository public. Please do not post your email, real name, phone number, school, organization, device details, qualification code, device token, or any secret in a public Issue.

A maintainer reviews the Issue and replies with next steps. Qualification codes are one-use secrets and must never be published in an Issue. If approved and a private delivery channel is needed, the applicant may voluntarily email the Issue link to [coolhezi@sjtu.edu.cn](mailto:coolhezi@sjtu.edu.cn); email is for private contact, not a substitute application. On receiving a code, open the [hosted login page](https://gatherthread.cn/app/), enter your display and device names, and activate it under **First-time activation**. Save the separately issued device token privately for later sign-ins. Existing qualified users can invite guests into individual projects without granting them account-wide project creation.
