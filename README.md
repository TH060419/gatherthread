...
The hosted Alpha runs source commit `52aac1a5ffda`, now identified by the `v0.1.0-alpha.7` Git tag. It includes the remembered-account chooser, first-entry cloud Git notice, and per-user cloud-code cleanup/quota. The already-published Alpha 7 npm packages were not rebuilt when the Git tag was aligned with the server source; no Alpha 7 GitHub Release has been created.
