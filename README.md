...
The remembered-account chooser, first-entry cloud Git notice, and per-user cloud-code cleanup/quota are currently **Unreleased source changes**. The hosted Alpha may still run the preceding release until a reviewed deployment is completed.

## Request Alpha test access

**[Open an Alpha access Issue](https://github.com/TH060419/gatherthread/issues/new?template=test-access.yml)** to request a test qualification. This Issue is the only application channel; there is no separate application form or server-side applicant profile. While the repository is private, the Issue link is accessible only to people with repository access; public requests become possible after the owner makes the repository public. The application can optionally describe why you want to join, what you want to test, and how you heard about GatherThread. Email is optional and recommended if you are comfortable sharing it in a public Issue and want an approved code sent privately by email. If you prefer not to share your email publicly, after approval you may privately email the Issue link to [coolhezi@sjtu.edu.cn](mailto:coolhezi@sjtu.edu.cn). Issues are public; never post qualification codes, device access tokens, passwords, keys, or private source code.

A maintainer reviews the Issue and replies with next steps. Qualification codes are one-use secrets and must never be published in an Issue. On receiving a code, open the [hosted login page](https://gatherthread.cn/app/), enter your display and device names, and activate it under **First-time activation**. Save the separately issued device token privately for later sign-ins. Existing qualified users can invite guests into individual projects without granting them account-wide project creation.
