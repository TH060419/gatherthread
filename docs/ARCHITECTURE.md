...
**Import Codex history** uses a distinct requester-private `visible_history_replace` job and is allowed only for a writable session. The wire kind keeps its Alpha name for compatibility. The connector builds an external native history source from canonical events, inserts a local-only marker for an empty session, summarizes oversized history below the configured context high-water mark, and verifies a newly created task before changing the binding. The Hook registry removes the prior task's authorization and authorizes the new task in one atomic file update. The prior native task itself remains untouched for the user to review and archive. Automatic invocation is either once when each session is first created locally, which is the default, or disabled. Every manual request creates a new local task. Realtime delta injection remains a separate path in both modes and after a manual import. [ADR-0021](adr/0021-import-visible-codex-history-as-a-new-task.md) records this boundary.
