...
packages/adapters harness-specific Codex, Claude Code, and ZCode transcript parsers
packages/codex-connect standalone local Codex connector shell
packages/dsh-host opt-in DeepSeek Harness Host plugin
packages/zcode-connect standalone local ZCode headless connector shell
...
ZCode uses the same server ordering, permission truth, claim boundary, and harness-neutral adapter contract through the standalone `@gatherthread/zcode-connect` connector ([ADR-0024](adr/0024-add-zcode-as-a-third-harness-through-a-standalone-headless-connector.md)). The connector registers one execution runtime per writable session and runs each claimed Web Agent request once in a bounded `zcode app-server` child inside the workspace, publishing public commentary as `agent_progress` while the turn runs and one final `agent_response` from the protocol's `turn.completed` result; `turn.failed`, timeout, and deactivation always complete as bounded failures. Its compatibility modules resolve the CLI without a shell, probe required headless capabilities structurally plus a live protocol handshake, and refuse unsupported builds or protocol versions; shared canonical history — excluding the connector's own earlier output — is quoted into the prompt as untrusted data and continued through the protocol's session resume, with the binding and a write-ahead execution journal recorded in versioned private state so a crash replays instead of re-running a finished native turn. Revocation and shutdown abort in-flight children and refuse later publication. Tool events stay local unless the operator opts in and the tool name is on the reviewed allowlist. Hidden reasoning, ZCode's private session store, and GatherThread credentials never cross the boundary. Local direct-ZCode turn capture through reviewed Hooks is a planned later phase.
