...
While a DSH prompt is active, newly durable Host events produce bounded generic `agent_progress` heartbeats carrying no event content. This renews the exact claim without exposing reasoning, private streams, headers, or native metadata.
