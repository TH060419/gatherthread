...
- Each successful claim returns its server-assigned positive `attempt_count`. Current connectors echo it as `claim_attempt` on progress, completion, and request-linked tool events. The server rejects expired leases and superseded attempts, including an older execution from the same runtime. Omission is accepted only for attempt 1 so the Alpha wire change does not break an older connector immediately; a malformed present value fails closed.
