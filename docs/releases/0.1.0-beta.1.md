...
Install the generated tarball in a fresh Git-less temporary directory and verify the executable's `--help`, dependency resolution, local no-production-credential preflight, plugin structure, and MCP/runtime-tool isolation. Run `/Users/tonyhe/.codex/skills/.system/plugin-creator/scripts/validate_plugin.py plugins/gatherthread` on the release host.
