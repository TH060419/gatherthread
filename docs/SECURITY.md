...
| T5 | Runtime claim theft or duplicate work | bind request, user, device, session, and exact runtime; atomic claim; one active turn per runtime; lease renewal only by accepted progress; require the current claim attempt on progress, completion, and request-linked tool events; generate terminal-failure idempotency keys inside the server transaction | runtime claim, device-binding, exact-runtime reclaim, stale-attempt tool/progress/completion, single-slot, idempotency-poisoning, and bounded-recovery tests |
