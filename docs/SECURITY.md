...
| T5 | Runtime claim theft or duplicate work | bind request, user, device, session, and exact runtime; atomic claim; one active turn per runtime; lease renewal only by accepted progress; server attempt fencing rejects expired or superseded writers | runtime claim, device-binding, exact-runtime reclaim, stale-attempt, single-slot, and bounded-recovery tests |
