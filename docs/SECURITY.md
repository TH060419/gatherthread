...
- A dynamic DSH selection is eligible only when the authenticated runtime advertised the exact provider/model and requested reasoning effort. Legacy runtimes remain fixed-model, and claim matching never falls back to another profile.
...
- A manual history summary is a derived public version requested only by a current session writer from their own exact execution runtime. Its source IDs must name authorized public completed conversation messages in the same session. The server builds the bounded prompt and source digest and reserves its metadata from generic writes; only a completed server-linked response activates the version. A viewer may inspect summaries and originals but cannot generate one. The per-user project context policy does not grant write or runtime authority.
- Selected public message bodies remain untrusted input to the writer's local Agent. The summary prompt explicitly treats them as data, but model compliance is not a security boundary; writers should review sources and generated text before relying on the lossy version. The derived-context endpoint fails on unauthorized or oversized reads instead of falling back to raw history or silently clipping it.
...
- A GatherThread-selected DSH model and effort are applied at the public Agent prompt boundary for that turn only. The override is cleared after settlement so it cannot silently change a later DSH-native turn.
...
The server enforces active-device/project ACL, branch ownership, owner-only integration, expected-head comparison and idempotency. Only bounded regular-file snapshots, portable relative paths and executable flags are accepted. Private harness state, unsafe paths and recognized credentials are rejected. Submitted paths never become shell commands, repository URLs or server checkout hooks. Git subprocesses have bounded input/output, disabled prompts/hooks and isolated configuration; three-way conflicts leave authoritative heads unchanged. Separate file, branch, mutation and storage quotas apply.

Portable validation rejects malformed UTF-16, Windows device names and NTFS short-name-shaped aliases, and checks HFS-ignored formatting characters when identifying private paths and duplicate names. Normal Unicode and emoji source names remain supported. A shared credential predicate runs locally before source transmission and again on the server before Git persistence; it covers known GatherThread, provider, Git-hosting and private-key formats, not every possible secret. Source review remains required before sharing.

Local access is directory-scoped, excludes symlinks/private state and respects Git ignore rules. Downloads require an acknowledged unchanged baseline, recheck destinations, retain a recovery journal and refuse ignored-file collisions. Recovery creates a new sibling directory and never deletes/rebinds native tasks. Automatic upload pauses during observed Agent activity, unstable files, large deletions or interrupted downloads. These safeguards do not serialize tools in independent harness processes: concurrent Agents must use separate working copies.

Automatic upload verifies that the final inventory matches the inventory that passed the stability/deletion check. Incomplete or unknown Codex turns, pending Hook drafts and active execution journals count as busy even if top-level thread status says idle. Transient initialization failures can retry without resetting a binding; damaged bindings pause code sync without stopping conversations. Failed completion acknowledgements do not relabel successful local code operations as failures: retry uses the same job/receipt identity.

Code jobs use requester-private exact-runtime controls and carry no arbitrary local path. Job results never expose credentials or absolute local paths. A Web job cannot grant DSH local consent. Cloud deletion revokes code API access but does not delete local files; remaining Git objects/backups need operator retention management. Backup/restore must include both SQLite heads/ACL and their Git objects.
