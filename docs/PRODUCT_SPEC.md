...
## Optional project code collaboration (unreleased source preview)

An owner may enable a separate Git-backed code repository for a project. Code checkpoints are not conversation events: each authenticated member owns one cloud branch across devices and harnesses, and only the project owner can review and merge an explicitly submitted head into shared main. All project readers can inspect source branches, including viewers; Solo does not narrow source-code visibility. Upload uses an expected base, so a stale device cannot overwrite a newer cloud revision.
