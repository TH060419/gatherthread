...
Unreleased settings add a per-user 128 MiB logical current-head cloud Git quota and project checklist. A member may clear their own branch, including while project Git is paused; the owner may clear their own branch or reset the entire project's cloud repository after reviewing the effect on collaborators. Both operations compare all relevant current heads and do not touch local Git. Clearing a personal branch does not remove its already merged content from shared main. Old server Git objects and backups are retained until separate operator-approved retention and garbage collection, so physical storage is not immediately released.
