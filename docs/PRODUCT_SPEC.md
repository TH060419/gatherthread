...
The Alpha 7 entry UI first asks the user to choose the existing-account, qualification-activation, or project-invitation path. Only then does it show the shared display/device-name fields and that path's credential form; Back returns to the path choices.
...
Alpha 7 entry improvements retain separate account-wide qualification and project-level invitation paths. Alpha test access is requested only through a GitHub Issue, with no public application form or server-side applicant-information collection. A user who explicitly selects **Remember this device** can choose that account in a browser-local quick-login menu; the menu restores the last display and device labels for review and editing before sign-in. Logging out ends the active session but retains the remembered choice until Forget, 30-day expiry, or device revocation/rotation. This is unsuitable for shared browser profiles because anyone using that profile can select a remembered account. First workspace entry on each device displays a required in-page cloud Git notice before other work; it needs no browser notification permission. Users acknowledge the project-wide code visibility, collaboration-only purpose, optional upload, and limits of disabling cloud sync.

## Project collaboration boundary

A project groups related sessions and is the stable membership, invitation, and local-agent binding boundary. Creating a project atomically creates only its owner membership; an owner or participant then creates an eligible first session explicitly. Existing sessions named `General` are preserved, but new and historical projects are never backfilled with one. A project invitation grants access to the project's current and future sessions. The project owner can rename the project, creates and renames `multi` sessions, switches their own sessions between `solo` and `multi`, and can change every other member between `participant` and `viewer`. Owners and participants create and rename only their own personal `solo` sessions. A participant or viewer may leave another person's project, immediately losing its sessions and runtime access; an owner cannot leave their own project without deleting it. If the departing member has a cloud Git branch, the remover must explicitly delete that branch or wait for the owner to review and merge its exact head to project main, then select the merged resolution. The owner may decline a merge; the member may delete their own branch and exit. Removal and branch release are one transaction, without changing local Git. A session creator or project owner may permanently delete that session's cloud copy; only the project owner may delete the whole cloud project. Neither operation deletes local workspaces or Agent conversations.
...
The server retains canonical events. Each local harness may build a private projection or compacted summary for its own context window. Native compactions are not canonical shared history. A client may record only metadata such as `covers_through_seq` and the local projection ID.

A session writer may also select public, completed conversation messages and ask their own connected local Agent to generate a **shared manual history summary**. It is a separate, attributed and explicitly lossy derived version linked to exact source IDs. Every project member can read it, switch between summary and original display, and inspect earlier or overlapping versions; both originals and completed summary text remain selectable for another generation. The writer can edit the default summary instructions in Web settings. A per-user project setting chooses whether future Agent context reads use the summary view (default) or original text. The summary view replaces selected source IDs using the newest non-overlapping completed versions, expands nested-summary ancestry to avoid injecting both a summary and its source, and restores originals skipped by an overlapping older version. Native Codex/DSH context management and canonical history remain independent. See [ADR-0027](adr/0027-shared-manual-history-summaries.md).
...
ZCode joins as a third harness through the standalone `@gatherthread/zcode-connect` connector (see [ADR-0031](adr/0031-add-zcode-as-a-third-harness-through-a-standalone-headless-connector.md)). The user selects it per project like Codex, copies a credential-free connector command, and keeps the terminal process running; every writable session registers one exact ZCode runtime, and a claimed Web Agent request runs once in a bounded headless ZCode app-server child inside the workspace, sharing public commentary and the final answer while hidden reasoning never leaves the child; redacted tool events are shared only when the operator explicitly opts in, and only for tools on the reviewed allowlist. The connector quotes shared canonical history into each request as untrusted data, resumes the same native session across turns, and never reads ZCode's private session store. Direct local ZCode turns stay local until the reviewed-hooks upload path ships; ambiguous or offline ZCode runtimes never fall back to another harness.
...
Local filesystem access requires separate opt-in (`--code-sync` with reviewed Codex Hooks, or DSH's per-project native consent). Automatic code upload defaults off and waits for known Agent activity to stop and files to stabilize. It never auto-merges or auto-downloads. Manual downloads require an unchanged acknowledged local baseline; recovery creates a new sibling folder without deleting or rebinding old files or conversations. A lost original workspace can be recovered without starting Codex. Existing context injection, conversation upload controls and model selection remain independent.

The first implementation uses bounded authenticated checkpoint APIs over actual server Git storage, not a native Git remote or task-worktree orchestrator. Simultaneous cross-harness writers remain unsupported. See [CODE_SYNC.md](CODE_SYNC.md) for limits, workflows and test instructions, and [ADR-0025](adr/0025-opt-in-git-backed-code-checkpoints.md) for the storage decision.

Alpha 7 settings add a per-user 128 MiB logical current-head cloud Git quota and project checklist. A member may clear their own branch, including while project Git is paused; the owner may clear their own branch or reset the entire project's cloud repository after reviewing the effect on collaborators. Former members can see and explicitly clear only their own branches orphaned by earlier releases; these remain charged until cleared. Both operations compare all relevant current heads and do not touch local Git. Clearing a personal branch does not remove its already merged content from shared main. Old server Git objects and backups are retained until separate operator-approved retention and garbage collection, so physical storage is not immediately released.
...
11. An unanswered agent request has an accessible answering indicator; it changes to queued when the exact selected local runtime is offline and disappears after the linked canonical response. A claim is leased and renewed by accepted progress. If its holder stops producing work, only that exact runtime may reclaim the request; GatherThread never changes the recorded device, harness, provider, model, or runtime silently. A lapsed claim no longer consumes the runtime's single active slot. Reclaim is bounded and fenced by a server attempt number: past the attempt budget the server appends and broadcasts one canonical failure response, and the timeline offers a retry only to the original requester.
...
18. A project owner can rename the project and can rename or switch their own sessions between `solo` and `multi`; a participant can rename their own Solo but cannot convert it to Multi. Subscribed Web clients converge on session title and mode changes without rebuilding history. New managed Codex tasks use an initial `<session> · MULTI|SOLO · GatherThread` label, and a newly materialized DSH Session starts with `<session> · 共序 · MULTI|SOLO`, so each harness can distinguish new shared conversations from plain local ones. Adopted and resumed DSH Sessions keep their existing local titles; all local Agent titles remain independently editable. Neither title may participate in session/thread identity or duplicate creation.
