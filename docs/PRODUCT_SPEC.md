...
11. An unanswered agent request has an accessible answering indicator; it changes to queued when the local runtime is offline and disappears after the linked canonical response. A claim is leased and renewed by accepted progress. If the runtime holding it stops producing work, an eligible runtime of the same user takes the request over, so a request survives the loss of the runtime that first claimed it and a returning runtime is not consumed by its own abandoned claim. Re-dispatch is bounded: past the attempt budget the request fails, the server appends the canonical failure response itself, and the timeline presents that failure with a retry that replays the request's exact recorded harness, provider, model, and runtime.
