...
Unreleased entry UI first asks the user to choose the existing-account, qualification-activation, or project-invitation path. Only then does it show the shared display/device-name fields and that path's credential form; Back returns to the path choices.
