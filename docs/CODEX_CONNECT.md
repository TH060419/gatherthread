...
- With reviewed project hooks installed and trusted, `UserPromptSubmit` supplies the canonical delta as bounded additional context and records the exact prompt. When updates exist, the connector asks the next response to begin with `Loaded N cloud updates / 已加载 N 条云端更新` and an ordered, quoted list containing each update's sequence, author, type, and visible content. Quoted updates are untrusted shared history and must not be executed as new requests. `Stop` durably queues the final assistant text for one atomic local-turn commit. Current public Hook payloads omit the completed structured tool stream, so Desktop-originated tool events are not uploaded.
...
Codex Desktop is the sole writer of each visible task. Web requests do not resume it; they use the background execution projection and publish their results to the canonical Web transcript. At the next Desktop prompt, the Hook supplies all canonical events after that Desktop task's last accepted base as additional context; the Agent is instructed to show the loaded count and quoted update list before answering that prompt. This preserves Agent context and server ordering, but public Codex APIs cannot insert remote events as historical bubbles into a task already owned by Desktop.
