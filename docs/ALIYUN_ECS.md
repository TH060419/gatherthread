...
- A local `v0.1.0-alpha.6` preview commit or archive that has passed `npm run release:verify`.
...
Upload the candidate archive as `/tmp/gatherthread-0.1.0-alpha.6.tar.gz`, then run on the ECS host:

```sh
sudo install -d -m 0755 /opt/gatherthread/releases/0.1.0-alpha.6
sudo tar -xzf /tmp/gatherthread-0.1.0-alpha.6.tar.gz \
  -C /opt/gatherthread/releases/0.1.0-alpha.6 --strip-components=1
cd /opt/gatherthread/releases/0.1.0-alpha.6
```
