...
const PACKAGE_SPEC = "@gatherthread/codex-connect@0.1.0-alpha.2";
...
test("repo marketplace exposes the real plugin and all install surfaces pin one release ref", async () => {
  ...
  const fixedCommand = "codex plugin marketplace add https://github.com/TH060419/gatherthread.git --ref v0.1.0-alpha.2 --sparse .agents/plugins --sparse plugins/gatherthread";
  const [html, english, chinese, rootEnglish, rootChinese, packageReadme, release, translations] = await Promise.all([
    ...
    readFile(path.join(root, "docs", "releases", "0.1.0-alpha.2.md"), "utf8"),
