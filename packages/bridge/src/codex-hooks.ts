...
export function renderCodexHookConfig(input: {
  ...
  const commandArguments = [
    ...
    "--registry",
    input.registryPath,
  ].map((value) => shellQuote(value, platform)).join(" ");
  const handler = (additionalContext: boolean) => ({
    type: "command",
    command: commandArguments,
    ...(platform === "win32" ? { commandWindows: `& ${commandArguments}` } : {}),
    timeout: 5,
    ...(additionalContext ? { additionalContextLimit: 2500 } : {}),
  });
  return {
    description: "GatherThread local hook relay. Review with /hooks before trusting.",
    hooks: {
      UserPromptSubmit: [{ hooks: [handler(true)] }],
...
export async function installCodexHookConfig(input: {
  ...
  try {
    ...
    if (current.description === "GatherThread local hook relay. Review with /hooks before trusting.") {
      // This file was generated by GatherThread. Replace its previous dynamic
      // relay definition instead of accumulating stale project-specific hooks.
      output = input.config;
    } else {
  ...
  try {
    if (await readFile(configPath, "utf8") === serializedOutput) return configPath;
  } catch (error) {
    if (!(error instanceof Error && "code" in error && error.code === "ENOENT")) throw error;
  }
  const temporary = `${configPath}.tmp-${process.pid}-${randomUUID()}`;
  await writeFile(temporary, serializedOutput, { mode: 0o600 });
