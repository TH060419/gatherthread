...
Each bound local conversation also has an independent upload preference while the connector runs in reviewed `--plugin-hooks` mode. Use `collaboration_get_local_sync_status` to inspect it. Use `collaboration_set_local_auto_upload` only after confirming the target session and the requested on/off state. Use `collaboration_upload_local_turns` only after explicit user approval: it scans completed native Codex turns and uploads eligible turns even when the trusted Hook did not run, missed the turn, or failed. Manual upload does not silently re-enable automatic upload. A disabled automatic-upload preference also gates trusted Hook outbox commits, so Hooks cannot bypass it.

`collaboration_import_codex_history` is a separate, explicitly mutating local operation. Use it only after the user confirms the target session and understands that it creates a new local Codex task. It compacts long history to the configured budget, verifies the imported task before switching, leaves the previous task for the user to archive, and keeps realtime canonical context injection enabled. It never uploads a local turn.
